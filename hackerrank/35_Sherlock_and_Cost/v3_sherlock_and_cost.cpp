...
void MaxCost::find_max_sum_v1()
{
	...
	for(unsigned int i=2; i<=N; i++)
	{
		...
		unsigned int max_prev_index = 0;
		for(unsigned int pval=1; pval<=B[i-1]; pval++)
		{
			int val1 = B[i], val2 = pval;
			int delta = max(pval-1, (unsigned int) abs(val1-val2));
			if(max_prev < arr[i-1][pval] + delta)
			{
				max_prev = arr[i-1][pval] + delta;
				max_prev_index = pval;
			}
		}
		if(debug_num == i)
		{
			cout <<"max value = " << max_prev << ", B[i] = " << B[i];
			cout <<", B[i-1]= " << B[i-1] <<", index= " << max_prev_index ;
			cout <<" arr_prev= " << arr[i-1][max_prev_index]<< endl;
		}
		unsigned int max_b_diff = max(B[i], B[i-1]) -1;
		for(unsigned int pval=1; pval<=B[i-1]; pval++)
		{
			...
			for(unsigned int pval=1; pval<=B[i-1]; pval++)
			{
				...
				if(max_diff > arr2[i][val])
				{
					...
					//if(arr2[i][val] == 1121 && i == 14 && val == 1)
						if(i == 5 && val == 1)
						{
						cout <<"prev_val = " << arr2[i-1][pval] <<", value= " << arr2[i][val];
						cout <<", prev index= " << pval <<", prev B= " << B[i-1] << endl;
						}
...
void MaxCost::compare_arr()
{
	for(unsigned int i=1; i<=N; i++)
	{
		for(unsigned int j=1; j<=B[i]; j++)
		{
			if(arr[i][j] != arr2[i][j])
			{
				cout <<"Mismatch at index= [" << i << ", " << j <<"] and B-Val-wg= " << arr[i][j] <<", B-Val-ok= " << arr2[i][j] << endl;
