...
*/
			...
			} else
			{
				...
				if(0) //if(path.size())
				{
					if(path[path.size()-1].dir == LEFT)
						path[path.size()-1].dir = UP;
					else if(path[path.size()-1].dir == UP)
						path[path.size()-1].dir = RIGHT;
					else if(path[path.size()-1].dir == RIGHT)
						path[path.size()-1].dir = DOWN;
					else if(path[path.size()-1].dir == DOWN)
						path[path.size()-1].dir = OFF;
				}		
			}
		}
