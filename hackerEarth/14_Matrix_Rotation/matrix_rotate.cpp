...
void Solution::get_new_pos(int &x, int &y, int offset)
{
	switch (d)
	{
		case LEFT:
				y = y-offset;
				if(y < start) { offset= start-y; y = start; d = DOWN; get_new_pos(x,y, offset);}
				break;
		case DOWN:
				x = x+offset;
				if(x > end) { offset= x-end; x = end; d = RIGHT; get_new_pos(x,y, offset);}
				break;
		case RIGHT:
				y = y+offset;
				if(y > end) { offset= y-end; y = end; d = UP; get_new_pos(x,y, offset); }
				break;
		case UP:
				x = x-offset;
				if(x < start) { offset= start-x; x = start; d = LEFT; get_new_pos(x,y,offset);}
				break;
	}
...
{
	const int row = v.size();
	const int col = v[0].size();
	int n = row;
	if(n<col)
		n = col;
	start = -1;
